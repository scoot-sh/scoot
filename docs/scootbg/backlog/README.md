...
1. [**Choosing dependencies**](resolved/dependencies-done.md) — RESOLVED
   2026-09-26, by measurement: plain `wayland-client`,
   `zune-jpeg` + `png` + `image-webp` directly, `fast_image_resize`, a
   hand-rolled CLI, `serde_json` for the socket, a line-format state
   file, and glibc `mallopt` + `malloc_trim` to return the heap
