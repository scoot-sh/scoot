...
| `app_id` | The toplevel's app id, or `""` before the toolkit has sent one. An X11 window's (under [`--xwayland`](configuration.md#xwayland)) is its `WM_CLASS` class -- `XTerm` for `xterm` -- falling back to the instance; X windows are listed like any other, with nothing on the wire to mark them. |
...
`spawn`, `cycle-column-width`, `set-column-width`, `toggle-fullscreen`, `set-fullscreen`, `quit` — change arrangement rather than where
...
- **The `--tty` GPU scanout tier can refuse a capture with a retry.**
  ...
  output (see [tty.md](tty.md)), so the second refusal is reachable in
  normal use --
