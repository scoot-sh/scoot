...
| `fullscreen` | Whether the window is fullscreen. While its column is focused it covers its output: `rect` equals that output's `rect`, and every other window on the output reports `visible: false`. Focused away, it keeps that size beside the focused column (and may still be partly `visible`). |
