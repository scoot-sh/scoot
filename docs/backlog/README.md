...
- [Scanout candidates + scanout-tranche feedback](./core/gpu-scanout-candidates.md) — high: the primary-plane half landed with the format gate (above); left is `Kind::ScanoutCandidate` for *overlay* planes (with the capture mark extended to cover them), per-surface scanout-tranche dma-buf feedback, and presentation's `zero_copy` flag.
