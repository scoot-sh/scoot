...
- [Scanout candidates + scanout-tranche feedback](./core/gpu-scanout-candidates.md) — high, unblocked (client fullscreen landed): the eligibility rule (which windows may be tried for the primary) and `Kind::ScanoutCandidate`, which gates *overlay* assignment (the primary has no kind check).
...
- [Deep popup chains crash or freeze the compositor](./resolved/popup-depth-bound-done.md) — RESOLVED 2026-09-23 (PR #TBD): popup chains are capped at 64, and the ways an open chain could be re-parented deeper are refused (`already_constructed`, `not_the_topmost_popup`, a parent with no live role object); a surface made a popup again no longer has its same-flush child inserted under its dead tree node.
