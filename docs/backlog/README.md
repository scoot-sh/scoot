...
- [Scanout candidates + scanout-tranche feedback](./resolved/gpu-scanout-candidates-done.md) — RESOLVED 2026-09-23 (branch `gpu-scanout-feedback`; the primary-plane half with the format gate above, PR #228): the fullscreen window covering an output gets per-surface feedback with a `scanout` tranche (the advertised pairs the primary plane accepts, `target_device` the display device) while the output may go direct, reverted at once when the covering window changes and after a 2 s hold for a lock/stream/translucency; `wp_presentation` reports `zero_copy` for the surface scanned out directly. Overlay-plane candidates split out below.
