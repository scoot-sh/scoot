...
- [wayland-backend's unbounded received-fd queue](./resolved/wayland-backend-fd-queue-done.md) — RESOLVED 2026-09-24 (PR #241) by repinning wayland-backend to the scoot-sh fork (`docs/forks.md`: 0.3.17 + `a39311b8`, via `[patch.crates-io]`), which disconnects a client leaving more than 128 received fds unclaimed. Dev VM: the PR #236 probe went from 999 fds with newcomers and `scootctl` shed to the attacker disconnected, 18 fds, everyone served. Costs a client on `wayland-client`'s pure-Rust backend that batches more than 140 fd-carrying requests per flush a disconnect (measured, documented; libwayland and winit clients cannot hit it); a harm-rule call left to review. `fd_pressure/tests/backend_queue.rs` fails if the patch is dropped.
...
- [Many light connections can still pressure the table and shed scootctl](./core/pressure-many-light-connections.md) — medium; cheapest fix is a lower pressure line for IPC accepts. Cheaper to reach since the wayland-backend queue bound: seven idle connections parking 128 fds each (no objects) reach the line, measured.
