...
- [Pin the budget cause by message in the two bypass-loop cap tests](./testing/bypass-loop-cap-cause-pin.md) — not a flake (never reddens): pressure and budget refusals post the identical code on the identical object, so under a pressured table these pass while guarding the wrong cause. Needs its own design decision against the helper's deliberate code-only stance.
