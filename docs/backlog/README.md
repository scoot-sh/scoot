...
- [GLES resize in place](./resolved/gles-resize-in-place-done.md) — RESOLVED 2026-09-23 (PR #PRNUM): a GLES resize reallocates only the renderbuffer, keeping the EGL context, shaders, client textures and device; a refused size falls back to a pinned rebuild, then refuses as before. Dev VM (LTO off): 3.95 ms → 15.7 µs per resize (pixman 11.3 µs); `--nested` drag 17.0 → 14.7 ms CPU per size, the rest being llvmpipe's frame at the new size.
