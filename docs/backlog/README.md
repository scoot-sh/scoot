...
- [A composited cursor blocks primary-direct](./core/gpu-direct-blocked-by-composited-cursor.md) — medium, found on Asahi 2026-09-25: with no cursor plane, a visible pointer means no fullscreen window ever gets a primary attempt; mpv went direct only with its pointer hidden (~60% less compositor CPU).
