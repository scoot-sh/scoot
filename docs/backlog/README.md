...
- [wayland-backend's unbounded received-fd queue](./resolved/wayland-backend-fd-queue-done.md) — RESOLVED 2026-09-24 (PR #PRNUM) by repinning wayland-backend to the scoot-sh fork (`docs/forks.md`: 0.3.17 + `a39311b8`, via `[patch.crates-io]`), which disconnects a client leaving more than 128 received fds unclaimed. Dev VM: the PR #236 probe went from 999 fds with newcomers and `scootctl` shed to the attacker disconnected, 18 fds, everyone served. Costs a Rust `wayland-client` that batches more than 140 fd-carrying requests per flush a disconnect (measured, documented). `fd_pressure/tests/backend_queue.rs` fails if the patch is dropped.
