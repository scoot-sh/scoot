...
impl<R: Read> Lines<R> {
    ...
    /// `Request::Type` and then nothing large ever again, which would be a
    /// megabyte held for the rest of that connection's life -- and up to
    /// `slots::MAX_CONNECTIONS` of them at once. Anything within one chunk
