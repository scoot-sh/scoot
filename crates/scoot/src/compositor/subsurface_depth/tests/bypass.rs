...
/// The check's one false refusal, pinned so it stays a decision: a surface's
/// height is a bound that is never lowered, so a surface that once had a
/// tall subtree is judged by it after the subtree is gone. Surface 0 had 60
/// levels below it; they are cut off, and it is then attached where 60
/// levels would not fit. No real client nests anywhere near deep enough to
/// meet this (see `subsurface_depth.rs`).
#[test]
fn a_surface_is_judged_by_the_tallest_tree_it_ever_had() {
    let mut fixture = Fixture::with_window();
