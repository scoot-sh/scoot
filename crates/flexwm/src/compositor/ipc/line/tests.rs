...
fn reads_one_line_at_a_time_leaving_the_rest() {
    ...
    assert_eq!(reader.next(MAX_REQUEST_BYTES), LineRead::Line);
    assert_eq!(reader.line(), b"{\"type\":\"version\"}\n");
    assert_eq!(reader.next(MAX_REQUEST_BYTES), LineRead::Line);
    assert_eq!(reader.line(), b"{\"type\":\"windows\"}\n");
    assert_eq!(reader.next(MAX_REQUEST_BYTES), LineRead::Eof);
...
fn the_limit_is_the_whole_line_newline_included() {
    ...
    assert_eq!(reader.next(10), LineRead::Line);
    ...
    assert_eq!(reader.next(10), LineRead::TooLong);
    assert!(
        reader.line().len() <= 10,
        "buffer grew to {}",
        reader.line().len()
...
fn the_limit_applies_to_each_line_not_to_the_connection() {
    ...
    for _ in 0..3 {
        assert_eq!(reader.next(6), LineRead::Line);
...
fn an_endless_stream_with_no_newline_stops_at_the_limit() {
    ...
    assert_eq!(reader.next(4096), LineRead::TooLong);
    assert!(
...
fn a_failing_read_closes_rather_than_looping() {
    ...
    assert_eq!(reader.next(MAX_REQUEST_BYTES), LineRead::Failed);
}
...
fn a_line_split_across_reads_keeps_what_arrived_so_far() {
    ...
    assert_eq!(reader.next(MAX_REQUEST_BYTES), LineRead::Incomplete);
    assert_eq!(reader.next(MAX_REQUEST_BYTES), LineRead::Incomplete);
    assert_eq!(reader.next(MAX_REQUEST_BYTES), LineRead::Line);
    assert_eq!(reader.line(), b"{\"type\":\"version\"}\n");
    assert!(matches!(
...
fn the_limit_holds_across_however_many_reads_a_line_takes() {
    ...
    assert_eq!(reader.next(10), LineRead::Incomplete);
    assert_eq!(reader.next(10), LineRead::Incomplete);
    assert_eq!(reader.next(10), LineRead::TooLong);
    assert!(
        reader.line().len() <= 10,
