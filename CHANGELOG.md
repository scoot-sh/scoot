...
- **Screenshots on the GPU renderer no longer grow scoot's memory.** With
  ...
  screenshots of a still screen could run scoot out of memory. `scootctl
  screenshot --no-cursor` did it, and so did screen-capture tools (`grim`,
  and anything else using `ext-image-copy-capture-v1`). Memory now stays
